...
export namespace SchoolBusRepository {
    ...
    function parseRoutineForOneType(json: any): Array<SchoolBusRoutine> {
        ...
        for (let route in json) {
            ...
            for (let routines of json[route]) {
                const times: Array<Date | null> = routines.map(it => parse(it, "HH:mm", new Date(0, 0, 0)));
    ...
    export function getNextByFromTo(from: Campus, to: Campus, dateTime: DateTimeInSemester): Option<SchoolBusRoutine> {
        ...
        if (isWorkingDay) {
            return getNextByFromToAtTime(from, to, extractTime(dateTime.dateTime), SchoolBusRoutineType.WorkingDay);
        } else {
            return getNextByFromToAtTime(from, to, extractTime(dateTime.dateTime), SchoolBusRoutineType.NonWorkingDay);
        }
    }
    ...
    export function getLastByFromTo(from: Campus, to: Campus, dateTime: DateTimeInSemester): Option<SchoolBusRoutine> {
        let isWorkingDay = DateTimeInSemesterService.isWorkingDay(dateTime);
        if (isWorkingDay) {
            return getLastByFromToAtTime(from, to, extractTime(dateTime.dateTime), SchoolBusRoutineType.WorkingDay);
        } else {
            return getLastByFromToAtTime(from, to, extractTime(dateTime.dateTime), SchoolBusRoutineType.NonWorkingDay);
        }
    }
}
