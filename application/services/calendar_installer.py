...
SCHOOL_CALENDAR_DICT = {
    2016: (
        ((2016, 9, 5), 1, 12, 1, '2016-2017学年秋季学期', '2016_1'),
        ((2016, 11, 28), 1, 6, 2, '2016-2017学年冬季学期', '2016_2'),
        ((2017, 2, 13), 7, 12, 2, '2016-2017学年冬季学期', '2016_2'),
        ((2017, 3, 27), 1, 12, 3, '2016-2017学年春季学期', '2016_3'),
        ((2017, 6, 19), 1, 4, 4, '2016-2017学年夏季学期', '2016_4'),
    ),
    2017: (
        ((2017, 9, 11), 1, 9, 1, '2017-2018学年秋季学期', '2017_1'),
        ((2017, 11, 20), 10, 12, 1, '2017-2018学年秋季学期', '2017_1'),
        ((2017, 12, 11), 1, 8, 2, '2017-2018学年冬季学期', '2017_2'),
        ((2018, 2, 26), 9, 12, 2, '2017-2018学年冬季学期', '2017_2'),
        ((2018, 3, 26), 1, 12, 3, '2017-2018学年春季学期', '2017_3'),
        ((2018, 6, 18), 1, 4, 4, '2017-2018学年夏季学期', '2017_4'),
    )
}
...
def install_yearly_calendar(year):
    date_pointer = datetime.date(year, 1, 1)
    delta = datetime.timedelta(days=1)
    while date_pointer.year == year:
        if Calendar.query.get(date_pointer) is None:
            cell = Calendar(date=date_pointer, dayoff=True)
            cell.save()
        date_pointer += delta


def install_weeks(year, weeks):
    start_date_tuple = weeks[0]
    start_date = datetime.date(*start_date_tuple)
    start_week = weeks[1]
    end_week = weeks[2]
    for week in range(start_week, end_week + 1):
        week_delta = week - start_week
        for days_delta in range(0, 7):
            date_delta = datetime.timedelta(days=days_delta, weeks=week_delta)
            date = start_date + date_delta
            cell = Calendar.query.get(date)
            cell.week = week
            cell.year = year
            cell.term = weeks[3]
            cell.dayoff = date.isoweekday() >= 6
            cell.save()


def install():
    for year in SCHOOL_CALENDAR_DICT.keys():
        if Calendar.query.filter_by(year=year).count() == 0:
            install_yearly_calendar(year)
        if Calendar.query.filter_by(year=year+1).count() == 0:
            install_yearly_calendar(year+1)
        weeks_list = SCHOOL_CALENDAR_DICT[year]
        for weeks in weeks_list:
            install_weeks(year, weeks)
