...
from application.services.calendar_installer import install
# from application.utils import this_course, this_term, this_week

calendar = Blueprint('calendar', __name__)


@calendar.route('/manage/install')
def calendar_install():
    install()


@calendar.route('/cells/<iso_date>')
def get_cell(iso_date):
    timetuple = tuple(map(lambda x: int(x), iso_date.split('-')))
    date = datetime.date(*timetuple)
    cell = Calendar.query.get(date)
    return jsonify(year=cell.year)


@calendar.route('/cells/now')
def get_cell_now():
    today = datetime.date.today()
    return get_cell(today.isoformat())
