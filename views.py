...
from SHUhelper.models import Comment, Students, User, Words, db
...
def validate(user, pw):
    s = requests.Session()
    r = s.get('http://ea.cc.shu.edu.cn/login', timeout = 30, proxies=proxies)
    token = re.search(r'value="(\S{40})">', r.text,flags=0).group(1)
    postData = {'_token': token,
        'username': user,
        'password': pw
    }
    r = s.post('http://ea.cc.shu.edu.cn/login', data=postData, timeout=30, proxies=proxies)
    if r.text.find('学号或密码错误') != -1:
        return '0'
    else:
        r = s.get('http://ea.cc.shu.edu.cn/', timeout=30, proxies=proxies)
        name = re.search(r'<h3>([\s\S]*)</h3>', r.text,flags=0).group(1)
        r = s.get('http://ea.cc.shu.edu.cn/logout', timeout=30, proxies=proxies)
        return name
...
@app.route('/quit-result', methods=['POST', 'GET'])
def quit_result():
    site = 'quit'
    if request.method == 'GET':
        if site in session and CACHE.get(session[site]) is not None and CACHE.get(session[site]+'islogin'):
            r = CACHE.get(session[quit]+'content')
            resp = make_response(render_template('quit-submit.html', r=r))
        else:
            return redirect(url_for('login',site=site,check='vali'))
        return resp
    elif request.method == 'POST':
        reason = request.form['reason']
        r = CACHE.get(session[quit]+'content')
        r['reason'] = reason
        certification = generate_quit_certification(r)
        resp = make_response(render_template('quit-result.html', certification=certification))
        return resp
