...
@conversations.route('/<conversation_id>/unread')
def unread_messages(conversation_id):
    conversation = Conversation.objects.get_or_404(id=conversation_id)
    unreadmessages = conversation.get_unread()
    if len(unreadmessages) != 0:
        if current_user.card_id != unreadmessages[0]['sender']['cardID']:
            conversation.messages.extend(conversation.unreadmessages)
            conversation.unreadmessages = []
            conversation.save()
    return jsonify(unreadmessages)
...
class ConversationAPI(MethodView):
    ...
    def post(self):
        ...
        if to_user is None:
            ...
            conversation = Conversation.objects(Q(to_user=to_user.id, from_user=current_user.id) | (Q(to_user=current_user.id, from_user=to_user.id))).first()
