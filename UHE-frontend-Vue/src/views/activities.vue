<template>
  <v-container fluid grid-list-xs>
    <v-layout row wrap>
      <v-flex xs12 v-for="(item,index) in items" :key="index" class="pa-1">
        <v-card class="green darken-3 white--text"  hover @click="$router.push(item.url)">
  ...
  data () {
    return {
      items: [{
        ...
        name: '校园安全地图',
        url: '',
        class: 'iconfont icon-baoan-copy',
        icon: ''
      }]
    }
  }
