import {simulateLogin} from './simulateLogin';
import 'mocha';
...
describe('模拟登录测试', () => {
    it('登录并拿到Cookie', async () => {
        const cookies = await simulateLogin('http://xk.autoisp.shu.edu.cn', process.env.STUDENT_ID, process.env.PASSWORD);
        expect(cookies[0].cookieString().indexOf('ASP.NET_SessionId')).not.equal(-1);
    });
    it('拿到的Cookie可以用于访问课程列表', async () => {
        const cookie = await simulateLogin('http://xk.autoisp.shu.edu.cn', process.env.STUDENT_ID, process.env.PASSWORD);
    ...
    it('成绩页面可以用同样的方式来登录', async () => {
        const cookie = await simulateLogin('http://cj.shu.edu.cn', process.env.STUDENT_ID, process.env.PASSWORD);
        let cookiejar = Request.jar();
