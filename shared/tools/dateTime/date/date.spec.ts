...
import {DayService} from "../day/day";
...
describe('Date 测试', async () => {
    it('能使用人类的格式创建一个日期对象', async () => {
        ...
        expect(DayService.dayNumberToChinese(date.getDay()).value).equals('二');
    });
    it('能合并日期和时间', async () => {
        const date = DateService.createDate(2019, 1, 1);
        const time = TimeService.createTime(10, 10);
        const dateTime = DateTimeService.mergeDateTime(date, time);
        expect(dateTime.getFullYear()).equals(2019);
        expect(dateTime.getMonth()).equals(0);
        expect(dateTime.getDate()).equals(1);
        expect(DayService.dayNumberToChinese(dateTime.getDay()).value).equals('二');
    ...
    it('能判断两个日期是否是同一天', async () => {
        const date1 = DateTimeService.mergeDateTime(DateService.createDate(2019, 1, 1), TimeService.createTime(10, 10));
        const date2 = DateTimeService.mergeDateTime(DateService.createDate(2019, 1, 1), TimeService.createTime(20, 20));
        expect(DateService.isSameDate(date1, date2)).true;
        const date3 = DateTimeService.mergeDateTime(DateService.createDate(2019, 1, 2), TimeService.createTime(20, 20));
